...
def multi_map(arr, n = 1, &prc)
  new_arr = arr.dup
  n.times { new_arr.map! { |ele| prc.call(ele)}}
  new_arr
end
...
def chain_map(val, arr)
  arr.inject(val) { |accu, prc| prc.call(accu) }
end
...
def proc_suffix(sentence, hash)
  sentence.split(' ').map do |word|
    suffixes = ''
    hash.each { |k, v| suffixes += v if k.call(word) }
    word + suffixes
  end .join(' ')
end
...
def proctition_platinum(arr, *procs)
  ...
  arr.each do |el|
    procs.each_with_index { |prc, i| (hash[i + 1] << el) && break if prc.call(el) }
  end
  # hash.sort_by { |k, v| k }
  hash
end
...
def procipher(sentence, hash)
  sentence.split(' ').map do |word|
    orig_word = word
    hash.each { |k, v| word = v.call(word) if k.call(orig_word)}
    word
  end .join(' ')
end
...
def picky_procipher(sentence, hash)
  sentence.split(' ').map do |word|
    orig_word = word
    hash.each { |k, v| (word = v.call(word)) && break if k.call(orig_word)}
    word
  end .join(' ')
end
